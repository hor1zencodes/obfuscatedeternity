...
#define DITHER_STRENGTH 1.0
    ...
    float g(float x){
        if (x <= 0.0) return 0.0;
        return x <= 0.00031308 ? 12.92 * x : 1.055 * pow(max(0.0, x), 1.0 / 2.4) - 0.055;
    }
    ...
    float flareY(float y){float t=clamp(1.0-(clamp(y,0.0,FLARE_HEIGHT)/max(FLARE_HEIGHT,EPS)),0.0,1.0);return pow(max(0.0, t),FLARE_EXP);}
...
void mainImage(out vec4 fc,in vec2 frag){
    ...
    for(int k=-TAP_RADIUS;k<=TAP_RADIUS;++k){
        ...
        float spd=max(abs(sin(tu)),0.02),u=clamp((basePhase-tu)/max(uDecay,EPS),0.0,1.0),env=pow(max(0.0, 1.0-abs(u*2.0-1.0)),0.8);
    ...
    float ax = abs(nx);
    float stMag = mix(ax, pow(max(0.0, ax), FOG_TILT_SHAPE), 0.35);
    ...
    float wL = pixW;
#endif
    float m0=pow(clamp(smoothstep(FOG_BEAM_MIN - wL, FOG_BEAM_MAX + wL, L), 0.0, 1.0),FOG_MASK_GAMMA);
    float bm=1.0-pow(clamp(1.0-m0, 0.0, 1.0),FOG_EXPAND_SHAPE); bm=mix(bm*m0,bm,FOG_EDGE_MIX);
    float yP=1.0-smoothstep(HFOG_Y_RADIUS,HFOG_Y_RADIUS+HFOG_Y_SOFT,abs(yPix));
    float nxF=abs((frag.x-C.x)*invW),hE=1.0-smoothstep(HFOG_EDGE_START,HFOG_EDGE_END,nxF); hE=pow(clamp(hE,0.0,1.0),HFOG_EDGE_GAMMA);
    float hW=mix(1.0,hE,clamp(yP,0.0,1.0));
    float bBias=mix(1.0,clamp(1.0-sPix,0.0,1.0),FOG_BOTTOM_BIAS);
...
export const LaserFlow: React.FC<LaserFlowProps> = ({
  ...
  useEffect(() => {
    ...
    const initialRgb = hexToRGB(color || '#FF79C6');
